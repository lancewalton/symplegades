...
class ArgonautTransformAlgSpec extends FlatSpec with MustMatchers {
  ...
  "delete" must """return Xor.Left when the path does not exist""" in {
    delete("z")(rootJson) mustBe a[Xor.Left[_]]
  }
  ...
  "insert" must """return Xor.Left when the path already exists""" in {
    val c = insert("x", parse("3"))(rootJson)
    c mustBe a[Xor.Left[_]]
  }
